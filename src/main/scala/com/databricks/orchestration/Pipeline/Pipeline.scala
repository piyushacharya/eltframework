...
object PipelineNodeStatus extends Enumeration {
  val None, init, Started, RegisterLensPart, Finished, Error, dead_end, Running, EVENT_TODO, todo = Value
}

class  PipelineGraph {
  val logger: Logger = Logger.getLogger(getClass)
  var nodes: mutable.Map[String, PipelineNode] = mutable.Map()
  var edges: mutable.Map[String, PipelineEdge] = mutable.Map()
  var status = "Active"
  ...
  def get_edge(edge_id: String): PipelineEdge = edges(edge_id)
...
class PipelineNode(name: String, task: Task) {
  ...
  var status: PipelineNodeStatus.Value = PipelineNodeStatus.None
...
class Pipeline(pipelineGraph: PipelineGraph) extends Callable[String]{
  ...
  def addSparkSession(sparkSession: SparkSession): Unit = this.sparkSession = sparkSession
